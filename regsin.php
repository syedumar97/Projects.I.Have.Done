...
{
    $results = pg_query_params($dbconn, 'INSERT INTO users (firstName, lastName, email, username, passwd) VALUES ($1,$2,$3,$4,$5)', array("$firstName", "$lastName", "$email", "$username", "$passwd"));
    $result = pg_query_params($dbconn, 'SELECT * FROM users WHERE username = $1 and passwd = $2', array("$username", "$passwd"));
    if (pg_fetch_row($result)) {
      echo "success\n";
    }
